       ...
       for(var i=0;i<this.faceVertexUvs[materialIndex].length;++i)
       {
         ...
         for(var vertexIndex=0;vertexIndex<aVertexIndices.length;++vertexIndex){
         	var aVertexIndex=aVertexIndices[vertexIndex];
          	    if(redundantVertexVectors[textureIndices[vertexIndex]]==face[aVertexIndex]||redundantVertexVectors[textureIndices[vertexIndex]]===undefined){
         	    redundantVertexVectors[textureIndices[vertexIndex]]=face[aVertexIndex];
         		face[aVertexIndex]=textureIndices[vertexIndex];
          	   }
              
         	    else{
                uvs[materialIndex].push(uvs[materialIndex][textureIndices[vertexIndex]*2]);
                uvs[materialIndex].push(uvs[materialIndex][textureIndices[vertexIndex]*2+1]);                                   var newIndex=Math.floor(uvs[materialIndex].length/2)-1;
                      redundantVertexVectors[newIndex]=face[aVertexIndex];
                      face[aVertexIndex]=newIndex;
                      textureIndices[vertexIndex]=newIndex
         	    }
         }
       }
